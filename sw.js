const CACHE='global-env-v2.8-engineering-ui-v1';
const TILE_CACHE='global-env-map-tiles-v1';
const SHELL=['./','./index.html','./manifest.webmanifest','./icons/app-icon.svg','./map-tier.js','./tiles/manifest.json','./assets/goldwind-logo.png','./assets/ui-engineering.css','./assets/ui-engineering.js'];
...
self.addEventListener('fetch',e=>{
 ...
 const isMapTile=u.hostname==='tile.openstreetmap.org'||u.hostname.endsWith('.tianditu.gov.cn');
 if(isMapTile){
  e.respondWith(caches.open(TILE_CACHE).then(async c=>{
   const cached=await c.match(e.request);
   try{
    const net=await fetch(e.request);
    if(net&&(net.ok||net.type==='opaque'))c.put(e.request,net.clone());
    return net;

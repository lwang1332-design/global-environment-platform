(() => {
  ...
  function meta(){const h=$('#loadingHours'),e=$('#loadingElapsed');if(h)h.textContent=totalHours?`${doneHours.toLocaleString()} / ${totalHours.toLocaleString()} h`:(doneHours?`${doneHours.toLocaleString()} h`:'Current窗口');if(e&&started)e.textContent=`${Math.round((Date.now()-started)/1000)} s`}
  ...
  function afterNetwork(){if(!running||activeFetches>0)return;stage(0,'done','GIS/定位请求已返回或已进入本地计算');stage(1,'done',doneHours?`环境数据已返回 ${doneHours.toLocaleString()} h；来源类型以结果页RAW/Fallback/EST为准`:'环境数据请求已返回；正在解析');progress(78);stage(2,'active',doneHours?`统一 ${doneHours.toLocaleString()} h 时间轴并检查缺测`:'统一时间轴并检查缺测');setTimeout(()=>{if(!running)return;stage(2,'done',doneHours?`${doneHours.toLocaleString()} h 已进入统一时间轴`:'时间轴已统一');progress(90);stage(3,'active','执行海盐 → Cl⁻沉降 → 盐库存 → 湿润/凝露 → ISO 9223');},80)}
  ...
  window.fetch=async function(input,init={}){
    ...
    try{
      if(url.includes('/api/direct')&&method==='POST'&&init?.body){const b=JSON.parse(init.body);requestKey=b?.year||b?.mode||'current'}
      else if(url.includes('archive-api.open-meteo.com')){const u=new URL(url);requestKey=(u.searchParams.get('start_date')||'current').slice(0,4)}
    }catch{}
    ...
    const track=running;
    if(track){activeFetches++;if(url.includes('/api/direct')){stage(1,'active','Direct：ERA5 / CAMS / CMEMS 请求中');progress(25)}else if(url.includes('open-meteo.com')){stage(1,'warn','Direct未返回完整数据，正在使用明确标记的Open-Meteo Fallback');progress(42)}else if(/topodata|elevation|gebco|gis/i.test(url)){stage(0,'active','正在解析GIS/陆海与距海信息');progress(15)}}
    try{
      const r=await rawFetch(input,options);
      if(track){try{const clone=r.clone();clone.json().then(d=>{const n=inferHoursFromJson(d,requestKey);if(url.includes('/api/direct')){const rawReady=!!(d?.weather?.time?.length&&d?.cams?.time?.length&&d?.ocean?.time?.length);stage(1,rawReady?'done':'warn',rawReady?`Direct已返回 ${n.toLocaleString()} h ERA5 + CAMS + CMEMS`:'Direct响应不完整，将按页面规则使用Fallback/EST')};}).catch(()=>{})}catch{}}
      return r;
    }catch(e){if(track){if(url.includes('/api/direct'))stage(1,'warn','Direct请求失败，平台将尝试Fallback/EST');else stage(1,'warn',`数据请求异常：${e.message}`)}throw e}
  ...
  const loading=$('#loading');if(loading){new MutationObserver(()=>{const hidden=loading.classList.contains('hidden');if(!hidden&&running){meta();return}if(hidden&&running&&!finalizing){finalizing=true;stage(2,'done',doneHours?`${doneHours.toLocaleString()} h 时间轴完成`:'时间轴完成');stage(3,'done','物理模型与工程判定完成');progress(100);clearInterval(timer);running=false;finalizing=false;}}).observe(loading,{attributes:true,attributeFilter:['class']})}
})();

...
- 点击地名搜索旁的“地图选点”，从任一页面跳转到地图。
- 点击地图，或拖动蓝色标记，自动更新项目经纬度和“已选点”坐标。
- 手机端可先拖动、缩放地图，将目标对准中心十字后点击“选取中心点”；仅平移地图不会改变已选坐标。
- “回到已选点”只调整地图视图，不修改项目位置。键盘聚焦地图后可用方向键平移、Enter 选取中心点。
- 地图选点与地名搜索、手动坐标输入保持同步；调整位置后，已有计算结果提示需要重新计算。

...
import {requestJson} from './sources-v328.js';
...
async function loadLand(){
  if(!landPromise){
    landPromise=fetch(LAND_URL,{cache:'force-cache',signal:AbortSignal.timeout(30000)}).then(async r=>{
...
async function siteElevation(lat,lon,siteLand,signal){
  ...
  try{
    ...
    const {data:d}=await requestJson(u,{signal,timeout:10000,retries:0});return number(d?.elevation?.[0]);
...
export async function fallbackGisContext(lat,lon,signal){
  ...
  for(const bearing of bearings){
    checkAbort(signal);await delay(0,signal);
  ...
  const elevation=await siteElevation(lat,lon,siteLand,signal);
  return {
    elevation,
...
export async function resolveGis(lat,lon,direct,signal){
  ...
  return fallbackGisContext(Number(lat),Number(lon),signal);
}

...
def test_historical_flux_contract():
    x=flux.build_historical_request(2025,18.3,109.26)
    assert x['dataset']=='cams-global-atmospheric-composition-forecasts'
    assert x['request']['date']==['2025-01-01/2025-12-31']
...
def test_job_round_trip_has_no_secret():
    token=flux._encode_job({'requestId':'abcde-12345','lat':18.3,'lon':109.26,'mode':'historical','year':2025,'dataset':'x','resolution':'x','historyBasis':'ARCHIVED_FORECAST_FLUX','back':0})
    x=flux._decode_job(token)
    assert x['lat']==18.3 and x['year']==2025

...
const out=await page.evaluate(async p=>{
  ...
  const a=result.coastalAssessment,cams=result.inputData?.cams||{};
  const fluxFields=['ssDry1','ssDry2','ssDry3','ssSed1','ssSed2','ssSed3','ssWetConv1','ssWetConv2','ssWetConv3','ssWetLs1','ssWetLs2','ssWetLs3'];
  const fluxFinite=Object.fromEntries(fluxFields.map(k=>[k,(cams[k]||[]).filter(Number.isFinite).length]));
  return {project:result.project,summary:{airSalt:result.summary.airSaltMean,engineeringCl:result.summary.clDepMean,pd:result.summary.meanSo2Dep,rh:result.summary.meanRh,t:result.summary.meanTemp,camsHours:result.summary.camsHours,proxySaltHours:result.summary.proxySaltHours,coverage:result.summary.coveragePercent},flux:{provenance:cams.autoSeaSaltFlux||null,finite:fluxFinite},assessment:a,run:result.run};
},p);
...
if((out.assessment?.sd?.camsFluxCoveragePercent||0)<90)process.exitCode=5;
...
if(Object.values(out.flux?.finite||{}).some(n=>n<8000))process.exitCode=8;

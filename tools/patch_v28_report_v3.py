...
anchor=" const riskBars=topEnv.map(([k,v])=>`<div class=\"riskbar\"><div><b>${esc(k)}</b><span>${v}/100 · ${level(v)}</span></div><i><em style=\"width:${v}%\"></em></i></div>`).join('');"
...
if anchor not in s:
    raise SystemExit('riskBars anchor not found')
...
s=s.replace(css_anchor,css_repl)
...
if needle in s:
    s=s.replace(needle, '<div class="small">阵风说明：ERA5当前请求若无直接阵风序列，报告按10 m风速 × ${params.gustFactor}进行工程估算，属于估算值而非现场实测阵风。</div>${windChart}${rainChart}${radChart}${pressureChart}${snowChart}</section>')
else:
...
s=s.replace(old,new)
s=s.replace('<tr><th>环境域</th><th>风险评分</th><th>等级</th><th>工程关注</th></tr>${scoreRows}', '<tr><th>环境域</th><th>核心数据</th><th>风险评分</th><th>等级</th><th>工程关注</th></tr>${scoreRows}')
...
s=s.replace("${topEnv.slice(0,5).map((x,i)=>`${i+1}. ${x[0]} ${x[1]}/100`).join('　')}", "${topEnv.slice(0,5).map((x,i)=>`${i+1}. ${x[0]}：${envCoreData[x[0]]||'N/A'}（风险 ${x[1]}/100）`).join('<br>')}")

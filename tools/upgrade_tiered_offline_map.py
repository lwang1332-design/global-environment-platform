...
# Integration version: tiered-offline-map-v4
idx=Path('index.html')
...
# so we must target the last closing body in the source file.
...
w=re.sub(r"const CACHE='[^']+';","const CACHE='global-env-v2.8-map-tier-v4';",w,count=1)
...
assert s.rfind(script_tag)>s.rfind('</script>')-len('</script>')

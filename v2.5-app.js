...
function packageEntries(){const entries=activePackages.map((p,i)=>({p,i,auto:packageTrigger(p)}));for(const x of entries)if(!(x.i in packageManual))packageManual[x.i]=x.auto>=2;return entries.sort((a,b)=>b.auto-a.auto||Number(packageManual[b.i])-Number(packageManual[a.i])||a.i-b.i)}
function renderPackages(){let n=0;const entries=packageEntries();$('packageGrid').innerHTML=entries.map(x=>{const {p,i,auto}=x,sel=packageManual[i];if(sel)n++;const txt=auto>=3?'强烈推荐':auto===2?'推荐':auto===1?'可选':'不触发',cl=auto>=3?'strong':auto===2?'auto':'';return `<div class="package ${sel?'selected':''}"><input type="checkbox" ${sel?'checked':''} onchange="packageManual[${i}]=this.checked;renderPackages();renderGap();renderResult()"><span class="tag ${cl}">${txt}</span><h4>${p[0]}</h4><p><b>适用：</b>${p[1]}<br>${p[2]}</p><small>推荐等级 ${auto}/3 · 已按等级排序 · 可人工覆盖</small></div>`}).join('');$('pkgCount').textContent=n+' 个升级包'}
